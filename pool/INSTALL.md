...
If you are the first time build mergepool, you could run `bash install/install_mergepool.sh` instead of exec these shell commands one by one.

```
cd /work
wget https://raw.githubusercontent.com/btccom/mergepool/master/install/install_mergepool.sh
bash ./install_mergepool.sh
```
...
git clone https://github.com/btccom/mergepool.git
cd /work/mergepool
...
Now create folder for mergepool, if you are going to run all service in one machine you could run `install/init_folders.sh` as below.

```
cd /work/mergepool/build
bash ../install/init_folders.sh
```

**create kafka topics**

Login to one of kafka machines, than create topics for `mergepool`:
...
Before you start mergepool's services, you need to stetup MySQL database and bitcoind/namecoind (if enable merged mining).
