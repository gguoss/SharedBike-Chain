...
{
    cout<<"EthashCPUMiner::workLoop."<<endl;
	...
	for (; !shouldStop(); tryNonce++, hashCount++)
	{
        ...
        RLPStream s;
        s.appendList(2);
        s << w.headerHash << (h64)(u64)tryNonce;
		//h256 value = h256((uint8_t*)&ethashReturn.result, h256::ConstructFromPointer);
		h256 value = sha3(s.out());
		value = sha3(value);
		if (value <= boundary) {
           // cout<<"gguoss---compute value:"<<value<<"   boundary:"<<boundary<<"--nonce:"<<tryNonce<<"header Hash:"<<w.headerHash<<endl;
            auto slution = EthashProofOfWork::Solution{(h64)(u64)tryNonce, h256()};
            //cout<<"gguoss---nonce:"<<(u64)slution.nonce<<endl;
            if (submitProof(slution)) {
			    break;
            }
        }
		if (!(hashCount % 100))
