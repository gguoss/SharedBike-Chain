...
std::tuple<h256, h256, h256> EthashClient::getEthashWork()
{
	...
	cout<<"--getEthashWork, WithoutSeal:"<<m_sealingInfo.hash(WithoutSeal)<<"seedHash:"<<Ethash::seedHash(m_sealingInfo)<<"boundary:"<<Ethash::boundary(m_sealingInfo)<<endl;
...
std::tuple<h256, h256> EthashClient::getSealHash()
{
	// lock the work so a later submission isn't invalidated by processing a transaction elsewhere.
	// this will be reset as soon as a new block arrives, allowing more transactions to be processed.
	bool oldShould = shouldServeWork();
	...
	if (!sealEngine()->shouldSeal(this))
		return std::tuple<h256, h256>(h256(), h256());
	...
	if (!oldShould && shouldServeWork())
		...
		m_remoteWorking = true;
	ethash()->manuallySetWork(m_sealingInfo);
	cout<<"--getSealHash, Seal hash:"<<m_sealingInfo.hash()<<"seedHash:"<<endl;
	return std::tuple<h256, h256>(m_sealingInfo.hash(), Ethash::boundary(m_sealingInfo));
}

bool EthashClient::submitEthashWork(h256 const& _mixHash, h64 const& _nonce)
{
	cout<<"submitEthashWork, _mixHash:"<<_mixHash<<"_nonce:"<<_nonce<<endl;
	ethash()->manuallySubmitWork(_mixHash, _nonce);
	return true;
}

bool EthashClient::submitAuxBlock(h256 const& hash, std::string const& auxpow)
{
    cout<<"sbmitAuxBlock, hash:"<<hash<<", auxpow:"<<auxpow<<endl;
    ethash()->manuallySubmitAuxBlock(hash, auxpow);
    return true;
}
