...
#include "Ethash.h"
//#include <libbitcoin/utilstrencodings.h>
...
Ethash::Ethash()
{
    ...
    cnote<<"------Ethash init.";
	m_farm.onAuxBlockFound([=](EthashProofOfWork::AuxBlock const& auxblock)
	{
        // to do: verify auxblock.hash and m_sealing.hash();
		m_sealing.setAuxPow(auxblock.auxBlock);
		if (!quickVerifySeal(m_sealing))
			return false;
		if (m_onSealGenerated)
		{
            std::cout<<"gguoss -----m_onSealGenerated:"<<true<<std::endl;
			RLPStream ret;
			m_sealing.streamRLP(ret);
			m_onSealGenerated(ret.out(), auxblock.auxBlock);
		}
        else {
            std::cout<<"gguoss ------m_onSealGenerated:"<<false<<std::endl;
        }
		return true;
	});
}
...
void Ethash::manuallySubmitAuxBlock(h256 const& hash, std::string const& auxpow)
{
    m_farm.submitAuxBlock(EthashProofOfWork::AuxBlock{hash, auxpow}, nullptr);
}
...
u256 Ethash::calculateDifficulty(BlockHeader const& _bi, BlockHeader const& _parent) const
{
    ...
    bigint o;
    bigint timediff = _bi.timestamp() - _parent.timestamp();
    std::cout<<"timestamp diff:"<<timediff<<"durationLimit:"<<durationLimit<<std::endl;
    if (timediff >= durationLimit) {
        if (timediff > 8 * durationLimit) {
            timediff = 8 * durationLimit;
        }
        o = _parent.difficulty() - _parent.difficulty() * (timediff - durationLimit) / durationLimit / 2016;
        o = o > minimumDifficulty ? o : minimumDifficulty;
    }
    else {
        o = (durationLimit - timediff) * _parent.difficulty() / durationLimit / 2016 + _parent.difficulty();
    }
    std::cout<<"gguoss---blk number:"<<_bi.number()<<"--calc difficulty:"<<u256(o)<<"parent difficulty:"<<_parent.difficulty()<<"current.difficulty:"<<_bi.difficulty()<<std::endl;
    return u256(o);
...
bool Ethash::quickVerifySeal(BlockHeader const& _bi) const
{
    ...
    dev::FixedHash<32u> hash(_bi.m_auxpow->getParentBlockHash().begin(), dev::FixedHash<32u>::ConstructFromPointer);
    // to do:
    bool ret = true;
    std::cout<<"gguoss pow verify---Ethash::quickVerifySeal, ret:"<<ret<<"m_auxpow's parent block hash:"<<hash<<"--boundary:"<<b<<std::endl;
	return ret;
}
