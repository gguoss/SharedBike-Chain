...
public:
	...
	virtual void onSealGenerated(std::function<void(bytes const& s, std::string const& auxpow)> const& _f) = 0;
...
public:
	void generateSeal(BlockHeader const& _bi) override
	{
		...
		if (m_onSealGenerated)
			m_onSealGenerated(ret.out(), _bi.s_auxpow);
	}
	void onSealGenerated(std::function<void(bytes const&, std::string const&)> const& _f) override { m_onSealGenerated = _f; }
	EVMSchedule const& evmSchedule(EnvInfo const&) const override;

protected:
	std::function<void(bytes const& s, std::string const&)> m_onSealGenerated;
};
