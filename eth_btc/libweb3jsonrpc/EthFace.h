        ...
        class EthFace : public ServerInterface<EthFace>
        {
            public:
                EthFace()
                {
                    ...
                    this->bindAndAddMethod(jsonrpc::Procedure("eth_getAuxBlock", jsonrpc::PARAMS_BY_POSITION, jsonrpc::JSON_ARRAY,  NULL), &dev::rpc::EthFace::eth_getAuxBlockI);
                    this->bindAndAddMethod(jsonrpc::Procedure("eth_submitAuxBlock", jsonrpc::PARAMS_BY_POSITION, jsonrpc::JSON_BOOLEAN, "param1",jsonrpc::JSON_STRING,"param2",jsonrpc::JSON_STRING, NULL), &dev::rpc::EthFace::eth_submitAuxBlockI);
                ...
                {
                    std::cout<<"eth_submitWorkI:------"<<std::endl;
                    response = this->eth_submitWork(request[0u].asString(), request[1u].asString(), request[2u].asString());
                }

                inline virtual void eth_getAuxBlockI(const Json::Value &request, Json::Value &response)
                {
                    (void)request;
                    std::cout<<"eth_getAuxBlockI:------"<<std::endl;
                    response = this->eth_getAuxBlock();
                }
                inline virtual void eth_submitAuxBlockI(const Json::Value &request, Json::Value &response)
                {
                    std::cout<<"eth_submitAuxBlockI:------"<<std::endl;
                    response = this->eth_submitAuxBlock(request[0u].asString(), request[1u].asString());
                }
                ...
                virtual Json::Value eth_getAuxBlock() = 0;
                virtual bool eth_submitAuxBlock(const std::string& hash, const std::string& auxpow) = 0;
