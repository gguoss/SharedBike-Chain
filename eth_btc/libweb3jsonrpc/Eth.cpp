...
Json::Value Eth::eth_getAuxBlock()
{
	try
	{
		Json::Value ret(Json::arrayValue);
		auto r = asEthashClient(client())->getSealHash();
		ret.append(toJS(get<0>(r)));
		ret.append(toJS(get<1>(r)));
		return ret;
	}
...
bool Eth::eth_submitAuxBlock(std::string const& hash, std::string const& auxpow)
{
	try
	{
		return asEthashClient(client())->submitAuxBlock(jsToFixed<32>(hash), auxpow);
	}
	catch (...)
	{
		BOOST_THROW_EXCEPTION(JsonRpcException(Errors::ERROR_RPC_INVALID_PARAMS));
