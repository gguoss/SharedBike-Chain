...
public:
    ...
    using AuxBlock = typename PoW::AuxBlock;
	...
	{
        std::cout<<"start miner. sealer:"<<_sealer<<std::endl;
		WriteGuard l(x_minerWork);
    ...
    using AuxBlockFound = std::function<bool(AuxBlock const&)>;
	...
	void onSolutionFound(SolutionFound const& _handler) {
        ...
        std::cout<<"onSlutionFound set _handler."<<std::endl;
    }

    void onAuxBlockFound(AuxBlockFound const& _handler) {
        m_onAuxBlockFound = _handler;
    }
	...
	{
        std::cout<<"submitProof, solution, _s nonce"<<_s.nonce<<std::endl;
	...
	bool submitAuxBlock(AuxBlock const& _p, Miner* _m) override
    {
        if (m_onAuxBlockFound && m_onAuxBlockFound(_p)) {
            if (x_minerWork.try_lock())
            {
                for (std::shared_ptr<Miner> const& m: m_miners)
                    if (m.get() != _m)
...
private:
    ...
    AuxBlockFound m_onAuxBlockFound;
