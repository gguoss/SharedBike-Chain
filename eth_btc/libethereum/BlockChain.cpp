...
BlockHeader const& BlockChain::genesis() const
{
	...
	if (!m_genesis)
	{
		...
		//cout<<"genessis block:"<<gb<<endl;
		UpgradeGuard ul(l);
		m_genesis = BlockHeader(gb);
		cout<<"genessis header:"<<m_genesis<<endl;
...
void BlockChain::init(ChainParams const& _p)
{
	...
	cout<<"blockchain init. chain params:"<<endl;
...
ImportRoute BlockChain::import(VerifiedBlockRef const& _block, OverlayDB const& _db, bool _mustBeNew)
{
    ...
    cnote << _block.info.hash() << ": Future time " << _block.info.timestamp() << " (now at " << utcTime() << ")";
	// Check it's not crazy
	if (_block.info.timestamp() > utcTime() + 1 && !m_params.otherParams.count("allowFutureBlocks"))
	{
		clog(BlockChainChat) << _block.info.hash() << ": Future time " << _block.info.timestamp() << " (now at " << utcTime() << ")";
		// Block has a timestamp in the future. This is no good.
		BOOST_THROW_EXCEPTION(FutureTime());
...
Block BlockChain::genesisBlock(OverlayDB const& _db) const
{
	...
	if (!_db.exists(r))
	{
		...
		cout<<"compute root hash:"<<ret.mutableState().rootHash()<<endl;
...
VerifiedBlockRef BlockChain::verifyBlock(bytesConstRef _block, std::function<void(Exception&)> const& _onBad, ImportRequirements::value _ir) const
{
	...
	BlockHeader h;
	RLP r(_block);
    cout << "---block RLP:" << r << endl;
	try
	{
		h = BlockHeader(_block);
        cout << "start setAuxPow r3:" << r.data().cropped(r[0].actualSize() + r[1].actualSize() + r[2].actualSize() + 3).toString() << endl;
        h.setAuxPow(r.data().cropped(r[0].actualSize() + r[1].actualSize() + r[2].actualSize() + 3).toString());
		cout << "BlockChain::verifyBlock --header number:" << h.number() << endl;
