...
void Client::init(p2p::Host* _extNet, std::string const& _dbPath, WithExisting _forceAction, u256 _networkId)
{
	...
	cout<<"genesisHash:"<<bc().genesisHash()<<"networkId"<<_networkId<<endl;
...
void Client::startSealing()
{
	...
	if (author())
	{
		m_wouldSeal = true;
		m_signalled.notify_all();
        cnote<<"set m_wouldSeal true.";
	}
	else {
        clog(ClientNote) << "You need to set an author in order to seal!";
        cnote<<"You need to set an author in order to seal!";
    }
}

void Client::rejigSealing()
{
    cnote<<"isMajorSyncing:"<<isMajorSyncing();
    cnote<<"wouldSeal:"<<wouldSeal();
    cnote<<"remoteActive():"<<remoteActive();
	if ((wouldSeal() || remoteActive()) && !isMajorSyncing())
	{
		...
		if (sealEngine()->shouldSeal(this))
		{
			...
			if (wouldSeal())
			{
				sealEngine()->onSealGenerated([=](bytes const& header, std::string const& auxpow){
					if (!this->submitSealed(header, auxpow))
						cnote << "Submitting block failed...";
				});
				cnote << "Generating seal on" << m_sealingInfo.hash(WithoutSeal) << "#" << m_sealingInfo.number();
...
bool Client::submitSealed(bytes const& _header, std::string const& auxpow)
{
    std::cout<<"----submitSealed----."<<std::endl;
	bytes newBlock;
	{
		UpgradableGuard l(x_working);
		{
			UpgradeGuard l2(l);
			if (!m_working.sealBlock(_header, auxpow))
				return false;
		}
		DEV_WRITE_GUARDED(x_postSeal)
			m_postSeal = m_working;
    ...
    std::cout<<"----import new block----."<<std::endl;
