...
void Block::resetCurrent(u256 const& _timestamp)
{
    ...
    std::cout<<"----block current time:"<<_timestamp<<std::endl;
...
bool Block::sealBlock(bytesConstRef _header, std::string const& auxpow)
{
	/*if (!m_committedToSeal)
		return false;
	...
	if (BlockHeader(_header, HeaderData).hash(WithoutSeal) != m_currentBlock.hash(WithoutSeal))
		return false;
    */
    std::cout<<"gguoss m_committedToSeal:"<<m_committedToSeal<<"_header WithoutSeal hash:"
        <<BlockHeader(_header, HeaderData).hash(WithoutSeal)<<", m_currentBlock hash:"<<m_currentBlock.hash(WithoutSeal)<<std::endl;

	cnote << "Sealing block!";
    m_parentBlock = auxpow;
    ...
    try {
        auto parentBlock = RLP(m_parentBlock, RLP::AllowNonCanon);
        std::cout << "-----parentBlock:" << parentBlock.data().toString() << std::endl;
        ret.appendRaw(parentBlock.data());
    }
    catch (...) {
        std::cout<<" -----throw out."<<std::endl;
    }
	ret.swapOut(m_currentBytes);
    std::cout << "m_currentBytes size:" << m_currentBytes.size();
    RLP test(&m_currentBytes);
    cout << "---test:" << test << std::endl;
    std::cout<< "m_currentBytes actual size:" << test.actualSize() << std::endl;
    auto parentBlock = test.data().cropped(test[0].actualSize() + test[1].actualSize() + test[2].actualSize() + 3).toString();
    std::cout << " ----2parentBlock: " << parentBlock << std::endl;
    std::cout << "-----r0:" << test[0].actualSize() << std::endl;
    std::cout << "-----r1:" << test[1].actualSize() << std::endl;
    std::cout << "-----r2:" << test[2].actualSize() << std::endl;
    std::cout << "-----r3:" << test[3].data().size() << std::endl;
	m_currentBlock = BlockHeader(_header, HeaderData);
	std::cout << "Mined " << m_currentBlock.hash() << "(parent: " << m_currentBlock.parentHash() << ")";
