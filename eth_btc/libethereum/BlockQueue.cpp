...
ImportResult BlockQueue::import(bytesConstRef _block, bool _isOurs)
{
    ...
    cnote << "Queuing block" << h << "for import...";
	...
	cnote << "Block" << h << "is" << bi.number() << "parent is" << bi.parentHash();
	...
	if (bi.timestamp() > utcTime() && !_isOurs)
	{
		...
		cnote << "OK - queued for future [" << bi.timestamp() << "vs" << utcTime() << "] - will wait until" << buf;
	...
	else
	{
		...
		else if (!m_readySet.count(bi.parentHash()) && !m_drainingSet.count(bi.parentHash()) && !m_bc->isKnown(bi.parentHash()))
		{
			...
			cnote << "OK - queued as unknown parent:" << bi.parentHash();
